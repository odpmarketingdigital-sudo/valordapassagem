...
export function Footer() {
  ...
  const interiorRoutes = routes.filter(
    (r) => r.destino === "São Paulo" && !popularRoutes.some((p) => p.slug === r.slug)
  );
    ...
    <footer className="border-t border-slate-200 bg-primary-dark text-white">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <p className="text-sm text-blue-200 leading-relaxed">
              Consulte preços médios, empresas e dicas para passagens de ônibus no Brasil.
              Não vendemos passagens — recomendamos plataformas parceiras.
            </p>
          </div>
            ...
            <ul className="space-y-2">
              {popularRoutes.map((route) => (
                <li key={route.slug}>
                  ...
                  >
                    Passagem {route.origem} → {route.destino}
                  </Link>
                </li>
              ))}
            </ul>
        ...
        <div className="mt-10 pt-6 border-t border-blue-800 flex flex-col sm:flex-row justify-between gap-4 text-xs text-blue-300">
          ...
          <p>Links de afiliado — podemos receber comissão por compras realizadas.</p>
        </div>
      </div>
    </footer>
  );
